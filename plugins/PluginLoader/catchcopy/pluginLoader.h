...
public slots:
    ...
    /// \brief Windows-only: handle a clipboard paste detected by the keyboard hook
    /// (queued from the low-level hook to the main thread). No-op on other OS.
    void clipboardPasteDetected();
private:
    ...
    /// \brief intercept Ctrl+C/Ctrl+X then Ctrl+V in Explorer/Desktop via a per-user
    /// keyboard hook and route to Ultracopier (Windows only; no registry/HKLM/UAC).
    bool catchPaste;
