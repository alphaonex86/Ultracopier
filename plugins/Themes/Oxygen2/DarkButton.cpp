...
void DarkButton::paintEvent(QPaintEvent * event)
{
    if(backgroundLeft.isNull() || backgroundLeft.height()!=height())
    {
        ...
        {
            backgroundLeft=background.copy(0,0,9,36);
            backgroundMiddle=background.copy(9,0,46,36);
            backgroundRight=background.copy(55,0,9,36);
            backgroundPushedLeft=backgroundPushed.copy(0,0,9,36);
            backgroundPushedMiddle=backgroundPushed.copy(9,0,46,36);
            backgroundPushedRight=backgroundPushed.copy(55,0,9,36);
        }
        else
        {
            backgroundLeft=background.copy(0,0,9,36).scaledToHeight(height(),Qt::SmoothTransformation);
            backgroundMiddle=background.copy(9,0,46,36).scaledToHeight(height(),Qt::SmoothTransformation);
            backgroundRight=background.copy(55,0,9,36).scaledToHeight(height(),Qt::SmoothTransformation);
            backgroundPushedLeft=backgroundPushed.copy(0,0,9,36).scaledToHeight(height(),Qt::SmoothTransformation);
            backgroundPushedMiddle=backgroundPushed.copy(9,0,46,36).scaledToHeight(height(),Qt::SmoothTransformation);
            backgroundPushedRight=backgroundPushed.copy(55,0,9,36).scaledToHeight(height(),Qt::SmoothTransformation);
        }
    }
    QPainter paint;
    paint.begin(this);
