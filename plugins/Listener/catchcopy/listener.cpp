...
void Listener::listen()
{
    ...
    if(server.listen())
    {
        // NOTE (Windows): the named pipe is now created manually with a custom
        // security descriptor inside ServerCatchcopy::winListen() — the pipe is
        // labelled Low integrity so a medium/low integrity Explorer can connect
        // even when Ultracopier runs elevated. This replaces the old plan that
        // was sketched here as a comment (CreateNamedPipe + SetSecurityInfo).
