...
class ServerCatchcopy : public QObject
{
    ...
    private:
        ...
        void registerClientSocket(QLocalSocket *clientSocket);
#ifdef Q_OS_WIN32
        /* On Windows the named pipe is created manually (instead of through
         * QLocalServer) so that an explicit security descriptor can be attached:
         * the DACL still restricts the pipe to the current user, but a Low
         * integrity label (no-write-up) is added so a medium/low integrity
         * Windows Explorer can always connect, even when Ultracopier itself runs
         * elevated (High integrity) after a UAC prompt. QLocalServer's
         * UserAccessOption does not set this label, which is why an elevated
         * Ultracopier became unreachable from Explorer and copies silently fell
         * back to the native Windows handler. */
        void *winPipeHandle;            ///< HANDLE of the idle listening instance
        void *winConnectOverlapped;     ///< OVERLAPPED* for the pending ConnectNamedPipe
        void *winSecurityDescriptor;    ///< PSECURITY_DESCRIPTOR shared by every instance
        QWinEventNotifier *winConnectNotifier;
        bool winListening;
        std::wstring winPipeName;
        bool winListen();
        bool winCreatePipeInstance(bool firstInstance);
        void winAcceptCompleted();
        void winClose();
        bool winBuildSecurityDescriptor();
#endif
    protected:
